...
for (pkg in config.BLOCKED_APPS) {
    try {
        ...
        Log.e(TAG, "Failed to exclude $pkg from VPN", e)
            ...
            // ---- hasan_vpn VpnTuner: MTU / IPv6 / DNS from SharedPreferences ----
            // Package com.hasan.hasan_vpn is the host app; class is on the same
            // process classpath when this plugin is path-overridden into the app.
            try {
                val tunerClz = Class.forName("com.hasan.hasan_vpn.VpnTuner")
                val load = tunerClz.getMethod("load", android.content.Context::class.java)
                load.invoke(tunerClz, this)
                val apply = tunerClz.getMethod("applyToBuilder", Builder::class.java)
                apply.invoke(tunerClz, builder)
                android.util.Log.i(TAG, "VpnTuner.applyToBuilder applied")
            } catch (e: ClassNotFoundException) {
                android.util.Log.w(TAG, "VpnTuner not on classpath (host app not hasan_vpn?)")
            } catch (e: Exception) {
                android.util.Log.w(TAG, "VpnTuner.applyToBuilder failed: ${e.message}")
            }
